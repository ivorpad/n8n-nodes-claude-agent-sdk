...
**Note:** When a module re-exports AND uses a type locally, both statements are required:

```typescript
// Re-export for consumers (does NOT bind in local scope)
export type { User } from "./user";
// Import for local use in this file
import type { User } from "./user";
```

This is NOT redundant — `export type { X } from` is a pass-through that doesn't create a local
binding. Do NOT flag this pattern as a violation.
